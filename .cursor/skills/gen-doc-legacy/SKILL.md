---
name: gen-doc-legacy
description: Analyze a source code repository and generate technical documentation for LEGACY architectures (server-rendered monoliths, PHP, ASP). Graph-first pipeline with parallel sub-agents.
...
Generate SRS and Basic Design documents from **legacy** codebases using the same **graph-first** pipeline as modern: Phase 0 freezes **GitNexus** + **Graphify** into `_work/graph/`, Phase 1 materializes analysis views with graph evidence (including mixed UI/SQL in single files), then Phases 2–3 follow `document-dependencies.yaml`. Progress is saved to disk after every step — runs can be paused and resumed at any point.
